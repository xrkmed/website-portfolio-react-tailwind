...
import { DiGulp, DiNodejs } from 'react-icons/di';
import {
  FaCertificate,
  FaCircle,
  FaClock,
...
import { SiMysql, SiWebpack } from 'react-icons/si';
...
const skillsMap: { [key: string]: JSX.Element } = {
...
function getSkillsIcon(skill: string) {
  return (
    skillsMap[skill.toLowerCase()] || (
      <>
        <BiSolidErrorCircle />
      </>
    )
...
function addFinishedCourse(course: ICourse) {
  return (
    <>
      <div className='flex flex-col h-full md:justify-between bg-opacity-60 bg-gray-900 rounded-xl m-4 p-0 min-w-fit md:min-w-0 md:w-2/4 lg:w-1/4 shadow-md shadow-zinc-700 md:shadow-none'>
        <div className='bg-gray-900'>
          ...
          <small className='text-gray-500 font-semibold italic'>
            LINGUAGENS E FERRAMENTAS APROFUNDADAS
          </small>
            ...
            {course.skills.map((skill) => {
              if (skill.icon) {
                return (
                  <>
                    <div
                      className='px-2 py-1 text-zinc-100 font-semibold'
                      data-tooltip-id='default'
                      data-tooltip-content={skill.name}
                    >
                      <img
                        src={skill.icon}
                        className='w-auto h-8 rounded-2xl'
                        alt={skill.name}
                      />
                    </div>
                  </>
                ...
                return (
                  <>
                    <div
                      className='px-2 py-1 text-slate-200 font-semibold'
                      data-tooltip-id='default'
                      data-tooltip-content={skill.name}
                    >
                      {getSkillsIcon(skill.name)}
                    </div>
                  </>
                );
              }
        ...
        <div className='flex flex-row ml-2 text-sm justify-between overflow-hidden text-ellipsis flex-wrap'>
          ...
          {course.hours ? (
            <>
              <div className='flex mr-2 font-extralight italic text-zinc-300'>
          ...
          {course.finishedAt ? (
            <>
              <div className='flex mr-2 font-extralight italic text-zinc-300'>
                <FaRegCalendarCheck className='mt-1 mr-1' />
                {course.finishedAt}
              </div>
            </>
          ) : (
        ...
        <li className='text-left pl-4 py-5 list-none'>
          <ul className='italic flex gap-2 text-zinc-400'>
            <FaSchool className='mt-1' /> Instituição de Ensino:
          </ul>
          <p className='pl-4 font-semibold text-ellipsis overflow-hidden'>
            @{course.institution} (
            <a
              onClick={() => window.open(course.institutionLink, '_blank')}
              rel='noreferrer'
              target='_blank'
          ...
          {course.teacher && course.teacherLink ? (
            <>
              <ul className='italic flex gap-2 text-zinc-400'>
                <FaGraduationCap className='mt-1' /> Ministrado por:
              </ul>
              <p className='pl-4 flex gap-2 font-semibold'>
                <FaInfoCircle className='mt-1' />{' '}
                <a
                  onClick={() => window.open(course.teacherLink, '_blank')}
                  target='_blank'
                  rel='noreferrer'
                  className='cursor-pointer text-blue-500'
                >
            ...
            <>
              <button className='bg-gray-900 hover:bg-gray-800 text-zinc-100 font-semibold py-2 px-4 rounded-md shadow-sm shadow-black'>
                <a
                  ...
                  rel='noreferrer'
                  className='flex text-zinc-100'
                  data-tooltip-id='link'
                  data-tooltip-content={course.certificateLink}
                >
                  <FaCertificate className='mt-1 mr-2' />
                  Certificado
                </a>
              </button>
            </>
...
export default function Courses() {
  ...
  useEffect(() => {
    setTimeout(() => {
      fetch('https://api.xrkmed.com/courses.json')
        .then((response) => {
  ...
  return (
    <>
      <div
        className='flex md:mx-auto flex-nowrap overflow-x-scroll md:flex-wrap md:justify-center md:overflow-x-hidden md:whitespace-normal'
        id='coursesList'
      >
        <Tooltip
          id='default'
          place='top'
          className='bg-gray-900 text-gray-100 rounded-lg p-2'
        />
        <Tooltip
          id='link'
          place='bottom'
        ...
        {loading ? (
          ...
          <div className='flex flex-col justify-center items-center text-stone-200 animate-pulse mt-4'>
            <h1 className='text-2xl font-light'>
              Ocorreu um erro ao carregar as certificaçoes.
            </h1>
