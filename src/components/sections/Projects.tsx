  ...
  return (
    <>
      <div className='w-10/12 md:w-1/3 bg-gray-900 rounded-2xl overflow-clip'>
        ...
        <div className='flex flex-col items-center md:flex-row gap-4 p-2'>
          <img
            src={project.thumbnail}
            className='self-center md:self-auto rounded-md w-32 h-32'
          />
          <div className='flex flex-col text-center md:text-left w-auto overflow-clip'>
            <span className='text-whiteAlpha-800 text-3xl md:text-2xl font-semibold mt-2'>
              {project.name}
            </span>
            <h2
              className='text-whiteAlpha-700 text-base md:text-sm max-h-48 md:max-h-max overflow-ellipsis font-light mb-4 tracking-tight italic'
            ...
            <div className='flex flex-col md:flex-row mt-auto bottom-0 justify-center items-center md:items-start md:justify-between mr-2'>
              <button className='text-blue-500 font-semibold hover:underline mr-2 mt-4 md:mt-0 mb-2 md:mb-0'>
                <a
                  ...
                  data-tooltip-content={project.url}
                >
                  <FaExternalLinkSquareAlt className='mt-1' /> View more
                </a>
              </button>
            </div>
          </div>
...
export default function Projects(): React.ReactElement {
  ...
  useEffect(() => {
    setTimeout(() => {
      fetch('https://api.xrkmed.com/projects.json')
        .then((response) => {
      ...
      <div className='flex flex-wrap justify-center gap-24 mt-4 antialiased'>
        {loading ? (
          ...
          (projects.length > 0 && (
            <>
              ...
              <div className='flex flex-col md:flex-row gap-2 text-zinc-200 justify-center items-center text-center w-10/12'>
                ...
                <h1 className='text-2xl'>
                  Mais projetos em{' '}
                  <a
                    href='https://www.github.com/xrkmed'
                    target='_blank'
                    rel='noreferrer'
            ...
            <h1 className='text-2xl font-light'>
              Nenhum projeto foi adicionado aqui ainda.
            </h1>
          )
        )}
      </div>
